var dashboardType=1;
var meterInfoLabelX=-60,meterInfoLabelY=-162;
...
function alarmMonitor() {
        ...
        $.ajax({
            ...
            $.each(alarmList.alarms, function(i, alarm){
                //只显示前三条
                if(index<3&&alarm.status==1){
                    var li='<li><a data-toggle="modal" data-aid="' + alarm.id + '" data-target="#alertModel"><div class="text-center link-block"><i class="fa fa-warning fa-fw"></i>'+"   "+alarm.mname+"   "+alarm.type+"   "+alarm.content+'</div></a></li>';
                    $("#alarms").append(li);//追加tab
                    toastr.error('告警信息：'+alarm.content,'')
                }
                index++;
            });
            var showAll='<li><div class="text-center link-block"><a href="/alarm"><strong>显示全部告警</strong><i class="fa fa-angle-right"></i></a></div></li>';
