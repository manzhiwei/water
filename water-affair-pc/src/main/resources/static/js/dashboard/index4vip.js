...
function alarmMonitor() {
        ...
        $.ajax({
            ...
            $.each(alarmList.alarms, function(i, alarm){
                //只显示前三条
                if(index<3){
                    var li='<li><a data-toggle="modal" data-aid="' + alarm.id + '" data-target="#alertModel"><div class="text-center link-block"><i class="fa fa-warning fa-fw"></i>'+"   "+alarm.meterName+"   "+alarm.alarmType+"   "+alarm.alarmContent+'</div></a></li>';
                    $("#alarms").append(li);//追加tab
                    toastr.error('告警信息：'+alarm.alarmContent,'')
                }
                index++;
            });
