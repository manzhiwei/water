$(function() {
	...
	// 数据模板
	var _onceT = true;
	function dataTemplate(type, data) {
		var _html = '';
		for (var i = 0; i < data.length; i++) {
			...
			var _url = "/history?id=" + data[i]["num"]+"&type="+type;
			console.log(_url);
...
//		}
	...
	// 首次加载默认 渲染瞬时流量
	ajaxDate("flow", "/queryMeterInfoForMobile", function() {
		if (data["0"]) {
			dataTemplate("flow", data["0"]);
		} else {
	...
	$(".ts-list").on(
			...
			function() {
				...
				if (_type == "spara") {
					...
					ajaxDate(_areaId, "/queryMeterInfoForMobile?id="
							+ _areaId, function(data) {
						// 渲染数据的类型 _dtype
						dataTemplate(_dtype, data);
					});
				}
				if (_type == "sarea") {
					if (_txt == _otxt)
